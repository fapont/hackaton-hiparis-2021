...
class Environment(gym.Env):
    ...
    def get_cost(self):
        return self.mg._df_record_cost['cost'][-1]
    ...
    def action_grid(self, mg, action):
        ...
        if action == 0:
            ...
            control_dict = {'pv_consummed': min(pv, load)+p_charge_pv+max(0, pv - min(pv, load) - p_charge_pv),
                            'battery_charge': p_charge_pv,
                            'battery_discharge': 0,
                            'grid_import': 0,
                            'grid_export': max(0, pv - min(pv, load) - p_charge_pv),
        ...
        if action == 4:
            load = load + p_charge_grid
            control_dict = {'pv_consummed': min(pv, load) + p_charge_grid + max(0, pv - min(pv, load) - p_charge_grid),
                            'battery_charge': p_charge_grid,
                            'battery_discharge': 0,
                            'grid_import': max(0, load - min(pv, load)),
                            'grid_export': max(0, pv - min(pv, load) - p_charge_grid),
        ...
        # Export
            ...
            control_dict = {'pv_consummed': min(pv, load) + abs(min(net_load, 0)),
                            'battery_charge': 0,
                            'battery_discharge': 0,
                            'grid_import': 0,
                            'grid_export': abs(min(net_load, 0)),
